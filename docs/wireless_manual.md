...
当前固件使用无线链路协议 `v1`。无线帧头偏移 2 的协议版本字段为 `01`，代码
中的唯一版本常量是 `RADIO_PROTOCOL_VERSION 1U`。非 `01` 版本的帧会被当前解析器
拒绝；当前项目未发布产品，不保留多版本接受路径。
...
无线链路采用包级自适应跳频，不依赖 Bluetooth 的 1600 hop/s 时钟槽。现有链路
是半双工停等模型，包级切换可以在没有连续同步时钟的情况下提供确定性恢复。
...
1. 主机在当前频道可靠发送 `HOP_SWITCH`。
2. 从机在当前频道回复 ACK，发完后切到目标频道并开始 360 ms 试用。
3. 主机收到 ACK 后切换，并在新频道可靠发送 `HOP_CONFIRM`。
4. 从机确认后回复 ACK，双方停留在新频道。
5. 任一方无法完成确认事务时回退。
...
[U5 原理图连接记录](schematic_u5_connections.md)为准；当前代码的绿/蓝 GPIO
与原理图记录存在差异，不能仅凭颜色判断映射已经正确。
...
射频控制脚的最新 MCU 映射和代码差异见[硬件手册](hardware_manual.md)，在
`RF_RX_EN`、`RF_TX_EN`、`RF_NRESET`、`RF_BUSY` 和 `RF_DIO1` 未完成校对前，
不得把冒烟失败归因于无线参数本身。
