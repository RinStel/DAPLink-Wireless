...
无线协议版本不等于固件发布版本。当前固件发布候选版本为 `0.8.0-rc.3`。
...
LED 通道与 MCU GPIO 的实际对应关系以[硬件手册](hardware_manual.md)和
[U5 原理图连接记录](schematic_u5_connections.md)为准；当前固件已按记录使用
`PC13/PC14/PC15`，但真实板卡的 LED 极性和电气行为仍需实测确认。
