...
| 目标供电使能 | `PA8`、`PB15` 两个独立接口 | `PA0` / `USB_AUTO_EN`，两个目标电源开关共用 | 有效电平和合并策略为 `待确认：` |
| 射频 NSS/SPI | `PA4/PA5/PA6/PA7` | `PA4/PA5/PA6/PA7` | 一致 |
| 射频 RX/TX 使能 | `PA0`、`PB13` | `PA1` / `RF_RX_EN`、`PA2` / `RF_TX_EN` | 不一致 |
| 射频复位 | `PB12` | `PA3` / `RF_NRESET` | 不一致 |
| 射频 BUSY | `PB11` | `PB1` / `RF_BUSY` | 不一致；当前 `PB11` 在原理图中为 `NC` |
...
这些差异只表示文档核对结果，不表示固件已经完成硬件同步。本次文档整理和
无线协议 v1 调整不修改当前代码中的板级 GPIO 宏。
...
3. USB 必须得到准确 48 MHz 时钟。8 MHz HXTAL 到 120 MHz 系统时钟的配置不能
   只通过 CPU 运行验证。
4. `USB_AUTO_EN` 同时连接 `U_TGT_SW1.EN`、`U_TGT_SW2.EN` 和 `Q2` 栅极。两个
   目标电源开关共用一个 MCU 网络；有效电平、目标供电策略和固件接口合并方式
   必须先确认。
...
在 U5 引脚映射确认并更新固件前，不得把这些宏名直接当作原理图网络名使用。
...
- `USB_AUTO_EN` 的有效电平、两个目标电源开关的行为及固件接口策略。
- `SW1`、`SW2` 的产品功能分工。
